...
/**
 * PaywallScreen — the UI + RC purchase machinery, extracted from the
 * old `app/(subscribe)/index.tsx` so it can back TWO routes without
 * copy-paste divergence:
 *
 *   - `app/(subscribe)/index.tsx`         — user-initiated (from home,
 *     favorite, or a locked story). Closes with `router.back()`.
 *   - `app/(paywall-onboarding)/index.tsx` — forced by the onboarding
 *     / cold-start gate. Closes with `router.replace("/(tabs)")` and
 *     shows an additional "Maybe later" dismiss below the fine print.
 *
 * The component parameterises the two things that differ between the
 * two routes:
 *
 *   - `source`          — analytics label ("subscribe_screen",
 *                         "post_onboarding", "cold_start_gate").
 *                         Fired in every logEvent call so the funnel
 *                         dashboard attributes conversions correctly.
 *   - `onDismiss`       — where to go when the user backs out.
 *   - `onPurchased`     — where to go on a successful purchase or
 *                         restore. Defaults to `onDismiss` when omitted
 *                         (matches the old subscribe screen behavior).
 *   - `showMaybeLater`  — render an extra "Maybe later" text link under
 *                         the sticky CTA. Used by the onboarding gate
 *                         so the escape hatch is obvious.
 *
 * All other behavior is byte-for-byte the same as the previous
 * subscribe screen — parental gate, trial timeline, plan selection,
 * savings badge, restore, legal links, error handling. Nothing about
 * the money-handling logic changed.
 */
...
/** Container scroll — was `SubscribeContainer` in the old file. Inlined here now. */
const PaywallScrollView = styled.ScrollView`
  flex: 1;
  background-color: ${designTokens.color.dark.bg};
`;

/**
 * `source` string is used for analytics attribution. The three current
 * values map 1:1 to the three ways this screen can be reached:
 *
 *   - "subscribe_screen"  → user tapped a Pro badge / locked story
 *   - "post_onboarding"   → gate right after `(profile-adventure)`
 *   - "cold_start_gate"   → recurring gate on cold start (until sub)
 */
...
export type PaywallScreenProps = {
  /** Analytics attribution — sent in every logEvent call from this screen. */
  source: PaywallSource;
  /**
   * Called when the user closes the paywall without purchasing.
   *   - subscribe route → router.back()
   *   - onboarding gate → router.replace("/(tabs)")
   */
  onDismiss: () => void | Promise<void>;
  /**
   * Called after a successful purchase OR restore. If omitted, falls back
   * to `onDismiss` (matches the pre-refactor behavior of the subscribe
   * screen, which just called `router.back()` on both dismiss and
   * successful purchase).
   */
  onPurchased?: () => void | Promise<void>;
  /**
   * Renders an additional "Maybe later" text link below the sticky CTA.
   * Used by the onboarding gate to make the escape hatch obvious (the
   * ✕ in the corner is easy to miss on a forced paywall).
   */
...
export default function PaywallScreen({
  ...
  /** After a successful purchase / restore, route the user forward. */
  ...
  useEffect(() => {
    loadOfferings();
    // `source` is stable per screen mount — don't re-fetch when parent
    // re-renders. The exhaustive-deps warning is intentional here.
  ...
  const handleClose = async () => {
    await logEvent("paywall_dismissed", { source });
    await onDismiss();
  };
    ...
    <View style={{ flex: 1, backgroundColor: t.color.bg }}>
      ...
      {!loading && !error && selected && (
        ...
        >
          ...
          {/* "Maybe later" — only rendered on forced-gate variants.
              The tiny ✕ in the corner is easy to miss, and Apple review
              expects a clear, tap-target-sized dismiss control on any
              paywall (Guideline 3.1.2). This is that control. */}
          {showMaybeLater && (
            <TouchableOpacity
              onPress={handleClose}
              hitSlop={12}
